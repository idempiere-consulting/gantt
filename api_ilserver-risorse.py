...
def translate_data_post(data_from_gantt,is_link=True):
    # arriva questo :
    ...
    tbl='lit_gantt_links' if is_link else data_from_gantt.pop('table_from') 
    ...
    for k,v in mapping[tbl].items():
        ...
        if isinstance(v,list):
            ...
            key_tr=v[1]
            # se il primo elemento è stringa vuota  vuol dire che il campo non me lo passa il gantt ma devo impostyarlo per esigenze di gantt
...
class LINK_add(Resource):
    ...
    def post(self):
        print('aggiungo link')
        r=request.values.to_dict()
        # {'source': '21000018', 'target': '21000020', 'type': '0'}
        print('\n*********************\link..',r)
        #devo sparare questo:
        # {"source": 21000019,"lit_target": 21000018, "lit_type": 2, "Name": "progetto"}

        api='post' + mapping['lit_gantt_links']['api']
        payload=translate_data_post(r)
