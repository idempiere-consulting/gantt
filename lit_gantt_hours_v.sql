...
SELECT 10000000::numeric + h.lit_hour_id AS lit_gantt_hours_v_id,
    ...
    h.name AS text,
    h.dateworkstart AS start_date1,
    h.dateworkstart + '01:00:00'::interval hour * h.qty::double precision AS end_date1,
    to_char(h.dateworkstart, 'yyyy-mm-dd hh24:mi:ss'::text) AS start_date,
    to_char(h.dateworkstart + '01:00:00'::interval hour * h.qty::double precision, 'yyyy-mm-dd hh24:mi:ss'::text) AS end_date,
...
UNION
 SELECT 90000000::numeric + c.c_contactactivity_id AS lit_gantt_hours_v_id,
    ...
    c.startdate AS start_date1,
    c.enddate AS end_date1,
    to_char(c.startdate, 'yyyy-mm-dd hh24:mi:ss'::text) AS start_date,
    to_char(c.enddate, 'yyyy-mm-dd hh24:mi:ss'::text) AS end_date,
